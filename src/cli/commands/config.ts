...
export async function runConfigCli(argv: string[]): Promise<void> {
  ...
  const subArgs = [...argv.slice(0, subPos.index), ...argv.slice(subPos.index + 1)];

  switch (subcommand) {
    case 'show': return runConfigShow(subArgs);
    case 'get': return runConfigGet(subArgs);
    case 'set': return runConfigSet(subArgs);
    case 'path': return runConfigPath(subArgs);
    case 'edit': return runConfigEdit(subArgs);
    case 'reset': return runConfigReset(subArgs);
    default:
      process.stderr.write(`seiton: config: unknown subcommand "${subcommand}"\nRun 'seiton config --help' for usage.\n`);
      process.exit(ExitCode.USAGE);
  }
}

function findConfigSubcommand(argv: string[]): { index: number; value: string } | undefined {
...
async function runConfigGet(argv: string[]): Promise<void> {
  ...
  if (args.positionals.length === 0) {
    process.stderr.write('seiton: config get: key path required\nUsage: seiton config get <key>\n');
    process.exit(ExitCode.USAGE);
  }
  ...
  const output = typeof result.value === 'object' && result.value !== null
...
async function runConfigSet(argv: string[]): Promise<void> {
  ...
  if (args.positionals.length === 0) {
    process.stderr.write('seiton: config set: key path required\nUsage: seiton config set <key> <value>\n');
    process.exit(ExitCode.USAGE);
  }
...
function parseConfigFlags(argv: string[], sub: string): ParsedConfigArgs {
  ...
  try {
    ...
    process.stderr.write(`seiton: config ${sub}: invalid arguments: ${detail}\nRun 'seiton config --help' for usage.\n`);
    process.exit(ExitCode.USAGE);
  }
